  ...
  "IvFailure Controller" must {
    ...
    "callback-failure route" when {
      ...
      "redirect to IV FallbackFailure when no journeyId is provided" in {
        ...
        redirectLocation(result).value mustEqual controllers.returning.routes.FallbackFailureController.onPageLoad().url
      ...
      "redirect to trust locked page when user fails Trusts IV after multiple attempts" in {
        ...
        redirectLocation(result).value mustEqual controllers.returning.routes.IvFailureController.trustLocked().url
      ...
      "redirect to trust utr not found page when the utr isn't found" in {
        ...
        redirectLocation(result).value mustEqual controllers.returning.routes.IvFailureController.trustNotFound().url
      ...
      "redirect to trust utr in processing page when the utr is processing" in {
        ...
        redirectLocation(result).value mustEqual controllers.returning.routes.IvFailureController.trustStillProcessing().url
    ...
    "locked route" when {
      ...
      "return OK and the correct view for a GET for not found route" in {
        ...
        val onLockedRoute = controllers.returning.routes.IvFailureController.trustNotFound().url
      ...
      "return OK and the correct view for a GET for still processing route" in {
        ...
        val onLockedRoute = controllers.returning.routes.IvFailureController.trustStillProcessing().url
