...
class IvFailureController @Inject()(
                                     ...
                                     lockedView: returning.TrustLocked,
                                     stillProcessingView: returning.TrustStillProcessing,
                                     notFoundView: returning.TrustNotFound,
  ...
  private def renderFailureReason(utr: String, journeyId: String)(implicit hc : HeaderCarrier) = {
    relationshipEstablishmentConnector.journeyId(journeyId) map {
      ...
      case UnsupportedRelationshipStatus(reason) =>
        ...
        Redirect(controllers.returning.routes.FallbackFailureController.onPageLoad())
      ...
      request.userAnswers.get(UtrPage) match {
        case Some(utr) =>
          ...
          queryString.fold{
            ...
            Future.successful(Redirect(controllers.returning.routes.FallbackFailureController.onPageLoad()))
          }{
            journeyId =>
