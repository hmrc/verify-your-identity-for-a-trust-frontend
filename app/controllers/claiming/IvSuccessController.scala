    ...
    def onPageLoad(): Action[AnyContent] = (identify andThen getData andThen requireData).async {
      implicit request =>
        ...
        request.userAnswers.get(UtrPage).map { utr =>
          ...
          lazy val onRelationshipNotFound = Future.successful(Redirect(controllers.claiming.routes.IsAgentManagingTrustController.onPageLoad(NormalMode)))
