...
class IvFailureController @Inject()(
                                     ...
                                     lockedView: claiming.TrustLocked,
                                     stillProcessingView: claiming.TrustStillProcessing,
                                     notFoundView: claiming.TrustNotFound,
  ...
  private def renderFailureReason(utr: String, journeyId: String)(implicit hc: HeaderCarrier) = {
    relationshipEstablishmentConnector.journeyId(journeyId) map {
      ...
      case UnsupportedRelationshipStatus(reason) =>
        ...
        Redirect(controllers.claiming.routes.FallbackFailureController.onPageLoad())
      ...
      request.userAnswers.get(UtrPage) match {
        case Some(utr) =>
          ...
          queryString.fold {
            ...
            Future.successful(Redirect(controllers.claiming.routes.FallbackFailureController.onPageLoad()))
          } {
            journeyId =>
